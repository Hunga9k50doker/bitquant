...
class ClientAPI {
  ...
  async makeRequest(
    ...
    do {
      try {
        ...
        if (errorStatus == 500) {
          return { success: false, status: error.status, data: null, error: "Code invalid or actived!" };
        }
  ...
  async handleActiveRefCode() {
    try {
      const refcode = refcodes[this.accountIndex] || getRandomElement(refcodes);
      const result = await this.activeCode(refcode);
      if (result.success) {
        this.log(`Actice code successfully: ${refcode}`, "success");
      } else {
        this.log(`Failed to active ref code: ${JSON.stringify(result)}`, "warning");
      }
    } catch (error) {}
  }
  ...
  async runAccount() {
    ...
    if (userData.success) {
      await sleep(1);
      await this.handleActiveRefCode(userData.data);
      await sleep(1);
    } else {
