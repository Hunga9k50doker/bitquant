...
class ClientAPI {
  ...
  async handleCreateRefCode() {
    let total = settings.AMOUNT_REF_CODE;
    let limit = total;
    while (limit > 0) {
      try {
        const result = await this.createCode();
        if (result.success && result.data.invite_code) {
          const reffCode = result.data.invite_code;
          this.log(`[${limit}/${total}] Created ref code successfully: ${reffCode}`, "success");
          fs.appendFileSync("reffCodes.txt", `${reffCode}\n`, "utf8");
        } else {
          this.log(`[${limit}/${total}] Failed to create ref code: ${JSON.stringify(result)}`, "warning");
        }
      } catch (error) {}
      await sleep(1);
      limit--;
  ...
  async runAccount() {
    ...
    if (userData.success) {
      await sleep(1);
      await this.handleCreateRefCode(userData.data);
...
async function main() {
  console.clear();
  showBanner();
  const privateKeys = loadData("privateKeysCreateReffCode.txt");
